...
template<class T>
EEPROMStorage<T>::EEPROMStorage(const uint8_t _blocks)
		: activeBlock(0)
		, blocks(_blocks)
	{
...
{	
    T tmp;
    EEPROM.get(START_ADDR + activeBlock * elementSize, tmp);

    uint8_t crc = crc_8((unsigned char*)&tmp, elementSize);
    uint8_t mark = EEPROM.read(flag_shift + activeBlock);
...
template<class T>
void EEPROMStorage<T>::clear(uint8_t block_index)
{
    for (int i = 0; i < elementSize; i++)
        EEPROM.write(START_ADDR + block_index * elementSize + i, 0);
}
