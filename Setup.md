...
| Название | Инструкция | Примечание |
| ---- | ---- | ---- |
| Электронная почта с сайта waterius.ru | Введите электронную почту указанную вами для доступа в личный кабинет. |  |
| Уникальный ключ | Оставьте поле пустым. ключ сгенерируется автоматически. |  |
| Адрес сервера | https://cloud.waterius.ru или [свой](https://github.com/dontsovcmc/waterius/blob/master/Export.md) | Если не заполнен, отправки не будет |
...
| Уникальный ключ | Вставьте из буфера обмена ключ из приложения Blynk. | Если поле пустое, отправки нет. |
| Адрес эл. почты | Blynk будет отправлять письмо каждые сутки | Если не заполнен, отправки не будет. Требуется виджет "эл. почта" в приложении |
| Заголовок | Заголовок эл. письма | {V0} - {V7} будут заменены на данные. Ограничение строки 64 символа. |
| Тело письма | Содержание | {V0} - {V7} будут заменены на данные. Ограничение строки 200 символов.  |
...
| V6 | светодиод - низкий заряд  |  |  |
| V7 | падение напряжения, В |  |  |
