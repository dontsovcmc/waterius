...
#
# Число - со слов владельца стенда: было 2,5 с, поднято до 3 с 25 сентября
# 2026, после того как отказ повторился при фактической выдержке 1,2 с.
# Механизм по-прежнему не установлен. В схеме
...
# «сеанс не доиграл».
#
# Взято не из выборки, а из железа: attiny снимает питание с ЕСП через
# WAIT_ESP_MSEC (`Attiny85/src/Setup.h`), две минуты, и для тревожного, и для
# передающего пробуждения - дольше сеанс не живёт физически. Плюс пять секунд
# на неспешность чтения лога. Подобранные по замерам числа тут уже подводили:
# тридцати секунд не хватило сеансу настроек, полсотни отрезали сеанс на 59 с
# (40 замеров этого стенда: 37 укладываются в 6-13 с, остальные 26, 49 и 59)
SESSION_TAIL_S = 125.0
