...
class Stand:
    ...
    def __init__(self, cfg: StandConfig, api: Metf, router: NatRouter,
        ...
        # Когда последний раз жали кнопку: по нему wait_session понимает, что
        # сеанс заказан и устройство обязано проснуться сразу
        self._press_at = 0.0
    ...
    def wait_session(self, timeout: float = 180.0, mode: int | None = None) -> Session:
        ...
        Сеанс, заказанный кнопкой, проверяется в два приёма. Устройство по
        кнопке просыпается сразу, поэтому первые строки обязаны прийти за
        `WAKE_FIRST_LINE_S`: их молчание - отказ устройства, и ждать после него
        минутами нечего. Когда лог пошёл, ждём его конца - строки ухода в сон, -
        дочитывая кусками: это уже про полноту сеанса, а не про пробуждение.
        Прежде обе беды сливались в один слепой таймаут «сеанс не пришёл за
        180 с», и разбирать его было нечем.
        """
        if self._press_at > self._session_at:
            self._press_at = 0.0
            if not self.log.wait_start(WAKE_FIRST_LINE_S):
                raise AssertionError(
                    f'Ватериус не проснулся по кнопке: за {WAKE_FIRST_LINE_S:.0f} с '
                    f'на UART ни одной строки при живой связи с METF. По кнопке '
                    f'устройство просыпается сразу, так что ждать дольше нечего - '
                    f'ищите нажатие: дошло ли оно до платы (строка «линия … '
                    f'расписка 202»), питание и сам шлейф')
    ...
    def settle_before_press(self) -> None:
        ...
        Нажатие вплотную к концу сеанса до устройства не доходит: стенд жмёт
        впустую и ждёт своего сеанса до таймаута. Механизм не установлен -
        долг записан в README; сама беда видна делом. Прежде
