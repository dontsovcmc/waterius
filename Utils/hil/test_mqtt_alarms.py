...
Отделено от базового MQTT (`test_mqtt.py`) по двум причинам. Первая - версии:
тревог нет ни в attiny младше 41, ни в ЕСП младше 2.0.47, и на такой прошивке
пропускаться должна эта группа, а не весь блок MQTT. Вторая - предусловие: без
порогов, доехавших до ОЗУ attiny, любой тест группы зеленеет на выключенных
тревогах, поэтому арматура вынесена в фикстуру `armed`, а не переписывается в
каждом тесте.

Требование к версиям - на модуле: `vac`, `af1` и `ackm` появились в посылке в
2.0.47 вместе со строкой `Alarm confirm` (история `json.cpp`, коммит 1ab57a8),
а `alarm_bits` в attiny - в 41.
"""
