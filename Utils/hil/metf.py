...
# Минимум для прогона - тринадцатый: он первый, где видны обе дыры в логе.
...
# 11 - заголовок `X-Uptime-Ms`) входят сюда же, и отдельно их не проверяют:
...
class Metf:
    ...
    def pulse(self, pin: int, value: int, duration_ms: int,
        ...
        растягивала паузы между импульсами: `D7` и проверка слипания начали
        видеть двойной счёт, потому что attiny перестал сливать два замыкания в
        один импульс.
        ...
        `wait=False` - вернуться сразу после расписки: выдержку и так держит
        плата, а у неё на выводы отдельные слоты. Так подаётся воздействие на
        один вход, пока замкнут другой; тому же выводу второй импульс до конца
        первого плата откажет кодом 409.
    ...
    def serial_read(self) -> str:
        ...
        assert seq is not None, (
            ...
            f'перед прогоном требует протокол {ACK_PROTOCOL} - значит, плату '
            f'подменили на ходу')
...
def check(api: Metf, host: str, stand_ssid: str = '', stand_channel: int = 0) -> str:
    ...
    """
    version = api.version()
    if version < ACK_PROTOCOL:
        raise MetfTooOld(
            f'METF {host}: протокол {version}, а нужен {ACK_PROTOCOL} и выше. '
            f'До двенадцатого плата осушала кольцо, собирая ответ: не доехавший '
            f'ответ уносил строки насовсем. До тринадцатого приёмный буфер UART '
            f'держал 22 мс, и голова сеанса пропадала молча - счётчика на это не '
            f'было. В обоих случаях тест падает на неполном логе не там, где '
            f'сломалось. Прошейте плату (репозиторий metf)')
