...
# Минимум для прогона - двенадцатый: он первый, где чтение лога подтверждается
# (`/read?ack=`), и плата не забывает строки, ответ с которыми не доехал.
# Прежние рубежи (8 - расписка `/pulse`, 9 - `/wifi`, 10 - `problem`,
# 11 - заголовок `X-Uptime-Ms`) в него входят, и отдельно их не проверяют:
...
# ожидание гарантировало переполнение - за него приходило больше строк, чем
# кольцо вмещает, и прогон терял тест с приговором «METF потерял N строк».
# Две секунды - половина времени набивки; повтор при этом бесплатен
READ_TIMEOUT_S = 2.0
...
def check(api: Metf, host: str, stand_ssid: str = '', stand_channel: int = 0) -> str:
    ...
    if version < ACK_PROTOCOL:
        raise MetfTooOld(
            ...
            f'ответ уносил строки насовсем, и тест падал на неполном логе не там, '
            f'где сломалось. Прошейте плату (репозиторий metf)')
