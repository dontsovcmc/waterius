...
# metf.
# Минимум для прогона - четырнадцатый: он первый, где формой сигнала
# распоряжается плата, а не стенд. До него паузу между фронтами набирал
# компьютер через Wi-Fi, и заказанные 0,3 с приходили как 2 с - проверка
# слипания замыканий винила в этом прошивку Ватериуса.
# Двенадцатый добавил подтверждение чтения (`/read?ack=`) - плата перестала
# забывать строки, ответ с которыми не доехал. Тринадцатый добавил `overruns`
# в `/read/stat` и поднял приёмный буфер UART до 4 КБ: до него байты пропадали
# в драйвере платы, до кольца, и ни один счётчик этого не показывал - сеанс
# терял голову при нулевом `dropped`.
#
# Прежние рубежи (8 - расписка `/pulse`, 9 - `/wifi`, 10 - `problem`,
# 11 - заголовок `X-Uptime-Ms`, 12 - подтверждение чтения, 13 - `overruns`)
# входят сюда же, и отдельно их не проверяют:
# поддерживать две прошивки дороже, чем прошить плату.
MIN_PROTOCOL = 14
...
#
# Поэтому срок теперь короткий, и это не нетерпение. Пока стенд ждёт мёртвый
# ответ, лог копится в кольце платы: 511 строк, а пик потока - 222 строки/с
# (05_air-and-loss.md), то есть кольцо набивается за 2,3 с. Четырёхсекундное
# ожидание гарантировало переполнение, двух секунд тоже не хватило: замер
...
class Metf:
    ...
    def __init__(self, host: str, attempts: int = ATTEMPTS,
        ...
        # Номер последней полученной строки лога: его стенд возвращает плате
        # параметром `ack`, и только тогда она забывает отданное. Пока номер
        # нулевой, плата не забывает ничего.
        self._log_seq = 0
    ...
    def wave(self, lines: list[dict[str, Any]], wait: bool = True) -> float:
        """
        Пачка фронтов одним запросом: `POST /pulse` телом JSON (протокол 14).

        Форму сигнала отмеряет плата, потому что всё, что между фронтами, стенд
        может отмерить только через радио. Заказанные 0,3 с так превращались в
        две секунды, и падал не стенд, а проверка прошивки Ватериуса.

        Ответ - расписка о приёме и полная длина пачки; её и выжидаем, как
        выжидали выдержку одиночного импульса. Что плата сделала на самом деле,
        отдаёт `pulse_stat` - и утверждать тест обязан по ней, а не по заказу.

        @return длина пачки в секундах
        """
        root = self._api._root
        session = self._api._sess
        answer = self._retry(
            'wave', session.post,
            (f'{root}/pulse',),
            {'json': {'lines': lines}, 'timeout': PULSE_ANSWER_TIMEOUT_S},
            repeatable=SAFE_TO_REPEAT)

        shape = ' | '.join(f'вывод {line["pin"]}: {line.get("value", 0)} на '
                           f'{line["edges"]}' + (f' через {line["at_ms"]} мс'
                                                 if line.get('at_ms') else '')
                           for line in lines)
        logger.info(f'пачка [{shape}]: расписка {answer.status_code} '
                    f'{answer.text.strip()}')

        if answer.status_code == 400:
            raise ValueError(f'стенд заказал негодную пачку: {answer.text.strip()}')
        answer.raise_for_status()
        if answer.status_code != 202:
            raise MetfTooOld(
                f'стенд: METF {self._host} ответила на пачку кодом '
                f'{answer.status_code}, а не 202: на плате прошивка старше '
                f'протокола 14, формы сигнала она не знает. Прошейте плату.')

        total_s = int(answer.text.strip()) / 1000.0
        if wait:
            time.sleep(total_s)
        return total_s

    def pulse_stat(self) -> dict[str, Any]:
        """Расписка последней пачки: моменты фронтов по часам платы."""
        answer = self._retry(
            'pulse_stat', self._api._sess.get,
            (f'{self._api._root}/pulse/stat',),
    ...
    def serial_read(self) -> str:
        """
        Забрать накопленный лог (`GET /read?ack=<номер>`).

        Свой метод, а не вызов через обёртку, из-за подтверждения. С протокола
        12 плата держит отданные строки, пока стенд не назовёт номер последней
        полученной, и потерянный ответ ничего не стоит: тот же запрос отдаёт то
        же окно, поэтому чтение повторяется по любому сетевому отказу.

        Плата старше 12-й ничего не придерживает: она осушает кольцо, собирая
        ответ, и не доехавший ответ уносит строки насовсем. Там повтор запрещён
        и каждая потеря считается дырой - иначе тест падал позже и не там:
        ...
        assert seq is not None, (
            f'METF не назвала номер окна лога ({LOG_SEQ_HEADER}): без него '
            f'подтверждать нечего, и потерянный ответ унесёт строки. Досмотр '
            f'перед прогоном требует протокол {MIN_PROTOCOL} - значит, плату '
            f'подменили на ходу')
        self._log_seq = int(seq)
        return answer.text
...
def check(api: Metf, host: str, stand_ssid: str = '', stand_channel: int = 0) -> str:
    ...
    Смертельно первое: протокол младше двенадцатого. Там чтение лога шло без
    подтверждения - плата осушала кольцо, собирая ответ, и не доехавший ответ
    уносил строки насовсем; тест падал на неполном логе не там, где сломалось.
    Прошивки помладше стенд не поддерживает вовсе: держать два пути чтения лога
    дороже, чем прошить плату.
    ...
    if version < MIN_PROTOCOL:
        raise MetfTooOld(
            f'METF {host}: протокол {version}, а нужен {MIN_PROTOCOL} и выше. '
            f'До четырнадцатого форму сигнала отмерял стенд через Wi-Fi: '
            f'заказанные 0,3 с приходили как 2 с, и проверка слипания замыканий '
            f'винила прошивку Ватериуса. До двенадцатого плата осушала кольцо, '
            f'собирая ответ: не доехавший ответ уносил строки насовсем. До '
            f'тринадцатого приёмный буфер UART держал 22 мс, и голова сеанса '
            f'пропадала молча. Прошейте плату (репозиторий metf)')
