...
# Длительность импульса. Опрос входа идёт раз в 250 мс, а замыкание attiny
# подтверждает повторным чтением через 50 мс (counter.h, discrete): опрос,
# попавший в последние 50 мс импульса, подтвердить его не успеет и сочтёт
# наводкой. При 300 мс в импульс попадал один опрос, и каждый пятый импульс
# терялся; при 500 мс опросов внутри всегда два, и первый заведомо не
# последний. Настоящий счётчик держит импульс дольше полусекунды
IMPULSE_WIDTH_MS = 500

# Пауза между импульсами. Конец импульса attiny видит по трём разомкнутым
# опросам подряд - это 750 мс, - и пока импульс не кончился, следующее
# замыкание не считается. 800 мс давали запас в полтакта, и импульсы терялись
# по одному на два десятка: на границе хватает джиттера сети
IMPULSE_GAP_S = 1.2
...
class Dut:
    ...
    def reset_board(self) -> None:
        """
        Сброс платы целиком, а не одной ЕСП: линия METF заведена на вывод
        reset у attiny, и импульс перезагружает обе. Проверено счётчиком
        сбросов attiny в посылке - 4 превращается в 5, - при том что ЕСП
        сообщает `esp restarted: 0`, то есть для неё это обычное включение.

        Тестам нужна обратная вещь: уронить ЕСП, не трогая attiny. Её даёт
        выход из режима настройки, см. `test_H7_esp_reset_keeps_period`.
        """
        self._low(self.reset_pin, RESET_MS)
