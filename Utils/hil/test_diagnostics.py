...
Плашка «внештатно перезагрузился» (код 22) здесь не проверяется: её поднимает
перезагрузка ЕСП без снятия питания, а не расход (docs/known-gaps.md). Сам флаг
перезагрузки проверяет `test_H7_esp_reset_keeps_period`.
"""
