...
- **Уходя, гасить радио.** `close()` шлёт `AT+CWQAP` и `AT+CWMODE=0` (null mode,
  RF выключено). Иначе плата остаётся станцией с запомненной точкой портала, а
  та существует только пока Ватериус в режиме настройки, - и плата ищет её
  вечно: `AT+CWAUTOCONN` по умолчанию `1` и хранится в NVS, `AT+CWRECONNCFG`
  отвечает `1,0` - раз в секунду, ноль попыток, то есть без предела, а
  `AT+RFPOWER:73` - это 18,25 дБм, почти максимум. Забытая в USB плата грелась
  так сутками без всякого стенда: модуль был горячим на ощупь, `AT+CWSTATE?`
  показывал `4,"waterius-..."`. Выключенное радио переживает перезагрузку
  (`AT+SYSSTORE:1`), обратно его включают `join()` и `wait_ready()`.
