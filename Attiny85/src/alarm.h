...
/*
Внеплановых сеансов на один период пробуждения.

ALARM_MAX_TRIES ограничивает попытки доставить ОДИН доклад, но не число
докладов: дребезжащий датчик протечки даёт новую новость на каждом переходе,
и каждая начинала бы счёт попыток заново. Пауза в ALARM_HOLD_MIN задаёт
только темп - двенадцать сеансов в час, - но не потолок, а сеанс Wi-Fi и есть
главная статья расхода батареи.

Поэтому потолок отдельный и общий: исчерпали - молчим до ближайшего планового
выхода на связь, он и увезёт текущее состояние. Худший случай перестаёт быть
"батарея за неделю" и становится "шесть сеансов вместо одного".
*/
#define ALARM_MAX_SESSIONS 5
...
struct AlarmReport
{
    ...
    uint8_t sessions : 3; // внеплановых сеансов с последнего планового
    uint8_t reserved : 1;

    AlarmReport() : tries(0), pending(0), sessions(0), reserved(0) {}
    ...
    */
    ...
    /*
    Остался ли бюджет внеплановых сеансов.
    */
    bool budget_left() const
    {
        return sessions < ALARM_MAX_SESSIONS;
    }

    /*
    Внеплановый сеанс состоялся.
    */
    void spend()
    {
        if (sessions < ALARM_MAX_SESSIONS)
            sessions++;
    }

    /*
    Плановый сеанс: он увёз состояние, бюджет начинается заново.
    */
    void new_period()
    {
        sessions = 0;
