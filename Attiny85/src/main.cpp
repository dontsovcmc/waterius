...
43 - 2026.09.12 - dontsov
	1. Смена типа входа снимает тревоги этого канала. Снять их после смены было
	   нечем: датчик протечки опрашивается, только пока тип входа - датчик, а
	   расход и ритм считаются по импульсам, которых у датчика не бывает.
	   Поднятый бит висел до перезагрузки attiny.

42 - 2026.09.06 - dontsov
	1. Протечка больше не срабатывает от одного импульса после долгой тишины.
	   Issue #405. Интервал между импульсами дольше ALARM_MAX_GAP_MIN считается
	   остановкой расхода, а не медленным ритмом: ритм начинается заново со
	   следующего импульса. Раньше такой интервал запоминался как ритм, и при
	   значении больше 32768 тиков условие непрерывного расхода становилось
	   истинным навсегда - ticks насыщается на 65535, и половина от него уже не
	   могла превысить запомненное. Тревога поднималась в тишине и не снималась.
