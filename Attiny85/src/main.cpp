...
41 - 2026.08.26 - dontsov
	...
	2. Новый тип входа LEAKAGE (5): проводной датчик протечки. Замыкание -
	   тревога, импульсы не считаются.
	...
	4. Тревоги уложены в свободные биты Header.flags: размер Header прежний.
...
Пауза после сеанса нужна в обоих случаях: для подтверждённого доклада это
защита от дребезга датчика протечки, для неподтверждённого - интервал до
следующей попытки.
*/
inline bool alarm_pending()
{
	return alarm_hold_min == 0 && (alarm_report.pending || alarm0.changed || alarm1.changed);
}
...
void loop()
{
	...
	*/
	...
	// Состояние держим, пока доклад не подтверждён: между попытками тревога
	// иначе успела бы погаснуть, и на сервер уехало бы "всё хорошо"
	alarm0.hold(alarm_report.pending);
	alarm1.hold(alarm_report.pending);
