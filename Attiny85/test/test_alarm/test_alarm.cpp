...
/*
Смена типа не выдумывает тревогу расхода на первом же импульсе.

Тревога расхода поднимается по времени с прошлого импульса. Если обнулять его
вместе с состоянием, отсчёт начнётся от момента настройки, и любой импульс
вскоре после неё окажется "короче порога".
*/
TEST(Alarm, TypeChangeDoesNotFakeFlowAlarm)
{
    AlarmDetector a;
    a.configure(TPM, 0);       // порог расхода - минута между импульсами

    idle(a, 10);               // десять минут тишины: настоящий интервал велик
    a.on_type_changed();
    a.on_pulse();              // первый импульс на новом типе
