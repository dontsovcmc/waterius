...
✅ [Тревоги](Alarms.md) — **экспериментальная функция**: много воды сразу, протечка,
остановка расхода, датчик воды на полу. Правила срабатывания ещё меняются, поэтому строить
на них единственную защиту от залива не стоит.
...
- вместо счётчика к входу можно подключить датчик протечки на полу: тревогой становится вода
  между электродами, а не только замыкание контактов ([Тревоги](Alarms.md))
...
**Симулятор портала** — страницы настройки открываются в браузере такими же, как на
устройстве, а импульсы, кнопка и Wi-Fi имитируются с пульта. Полезен, чтобы посмотреть
портал, не собирая прошивку: [simulator/README.md](simulator/README.md).
...
[Лицензия GNU LGPLv3](https://github.com/dontsovcmc/waterius/blob/master/LICENSE)
