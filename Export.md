...
| `<топик discovery>/<тип>/waterius-<id>/<сущность>/config` | только в сеансе после настройки в портале и после короткого нажатия кнопки | конфигурация сущности для Home Assistant |
...
Флаг retain у посылки и у discovery общий — это «Retain Flag» в настройках MQTT, по умолчанию
включён. Пустые сообщения, стирающие команды, публикуются retained всегда, независимо от флага.
