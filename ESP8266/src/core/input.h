...
строка и любой мусор — ошибка, а не молча обрезанное число.
*/
...
Список обязан совпадать с селектором в data/input_setup.html: тип, который
страница предлагает, а эта функция не признаёт, отрисуется обратно как NAMUR,
и сохранение страницы затрёт настройку пользователя.
*/
