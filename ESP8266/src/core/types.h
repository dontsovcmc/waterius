...
Сдвиг тут свой, не ATTINY_ALARM_SHIFT1: в Header.flags младший бит занят
флагом питания, а в маске снятия каналы лежат вплотную.
*/
#define ALARM_RESET_SHIFT1 3
#define ALARM_RESET_ALL 0x3F
#define ALARM_RESET_CH(bits, input) \
    ((uint8_t)((input) == INPUT0_RED ? (bits) : ((bits) << ALARM_RESET_SHIFT1)))
...
struct Settings
{
    ...
    */
    ...
    alarm_vol   - много воды сразу: литров за 30 минут.
    alarm_rate  - какой расход считать остановкой воды, л/ч.
    alarm_hours - сколько часов расход не падал ниже alarm_rate.

    Два последних читаются одним предложением: "расход не падал ниже
    alarm_rate л/ч в течение alarm_hours часов". Порознь ни одно из них
    смысла не имеет, поэтому ноль в любом выключает протечку целиком.

    Поля взяты из reserved9, поэтому общий размер не поехал: у прошитых
    устройств конфигурация читается как прежде, а новые поля приезжают
    нулями - тревоги по умолчанию выключены.
    */
    uint16_t alarm_vol0 = 0;
    uint16_t alarm_vol1 = 0;
    uint16_t alarm_rate0 = 0;
    uint16_t alarm_rate1 = 0;
    uint16_t alarm_hours0 = 0;
    uint16_t alarm_hours1 = 0;
    ...
    */
    ...
    Включён - порог "много воды сразу" подменяется одним импульсом, то есть
    тревогой становится любой расход, какой счётчик способен показать. Пороги
    пользователя при этом не затираются: выключил режим - вернулись прежние.

    Вес импульса режиму не нужен: незачем пересчитывать единицы, когда
    тревогой объявлен любой импульс. Поэтому он работает и на канале, где вес
    ещё не задан - иначе включённый режим молча не сработал бы там, где нужен
    не меньше.
    */
    uint8_t vacation = 0;
    ...
    */
    ...
    /*
    Отпечаток опубликованного автообнаружения (core/discovery.h). Ноль -
    ничего не опубликовано: так поле приезжает у прошитых устройств, и первый
    же сеанс после обновления переотправит конфиги.
    */
    uint32_t discovery_signature = 0;

    /*
    Зарезервируем кучу места, чтобы не писать конвертер конфигураций.
    Будет актуально для On-the-Air обновлений
    */
    uint8_t reserved9[27] = {0};
