...
{
    /*
    Вес импульса выбран пользователем, а не остался спецзначением из
    комбобокса ("Авто", "Как у холодной воды"). До первой настройки входа
    там лежит именно спецзначение, и сравнивать нечего.
    */
    bool factor_configured(const uint16_t factor)
    {
        return factor > 0
