...
strtol и три проверки: цифры вообще были, после числа нет мусора, результат
влезает в допустимые границы. Без них "abc" читается как ноль, 70000 в
uint16_t молча становится 4464 - порт MQTT сохранён другим, а портал ответил
"сохранено", - а "-1" в беззнаковом поле превращается в 255, то есть
"включено". Хвостовые пробелы разрешены: значение приходит из формы как есть.
*/
