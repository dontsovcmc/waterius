...
bool setClock(const char* ntp_server)
{
	...
	log_ntp.info(F("Waiting for NTP time sync: "));
...
bool setClock() 
{
	if (setClock("1.ru.pool.ntp.org") 
		...
		log_ntp.info(F("Current time: %s"),asctime(&timeinfo));
		return true;
	}
	return false;
}
