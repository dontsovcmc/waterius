...
{
    /*
    Единственный след кода ошибки в логе. Печатается здесь, а не рядом с
    вызовом в main.cpp, намеренно: строка обязана доказывать, что моргание
    действительно началось, а не что его собирались начать.

    Число вспышек этим не проверяется - стенд их не видит, у него нет счётчика
    фронтов (Utils/hil/README.md). Проверяется выбранный код, и одного этого
    мало: две разные поломки моргают одинаково - недоступное облако и
    недоступный свой сервер дают по три вспышки (cloud_status). Различить их
    можно только по строке Alarm confirm.

    На классике вывод светодиода - это линия TX, поэтому строка уходит до
    первой вспышки: дальше UART замолкает до restore_serial.
    */
