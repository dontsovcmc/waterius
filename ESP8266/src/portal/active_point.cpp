...
/*
Сколько порталу осталось жить: секунды до закрытия по бездействию.

Намеренно не кормит сторожевой таймер. Опрос состояния его тоже не кормит
(#305), и наблюдение за таймером тем более не должно его двигать: иначе
открытая вкладка держала бы устройство включённым, а проверка «действие
продлевает окно» стала бы проверкой самой себя.

Секунды, а не миллисекунды: на экране это минуты, а в проверке - разница
между «окно продлилось» и «нет», которая измеряется десятками секунд.
*/
