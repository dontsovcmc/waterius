...
void JsonConstructor::begin()
{
    ...
    _init = false;
    *_buffer = '\0';
    write(FPSTR(JsonConstructor_OpenObject));
}
...
void JsonConstructor::beginObject(const char *name)
{
    insert();
    write(name);
    write(FPSTR(JsonConstructor_NewObject));
    _init = false;
}
...
void JsonConstructor::beginArray(const char *name)
{
    insert();
    write(name);
    write(FPSTR(JsonConstructor_NewArray));
    _init = false;
}
...
{
    write(FPSTR(JsonConstructor_CloseObject));
}
...
{
    write(FPSTR(JsonConstructor_CloseArray));
}
...
{
    if (value)
        return write(FPSTR(JsonConstructor_True));
    return write(FPSTR(JsonConstructor_False));
}
...
uint16_t JsonConstructor::writeIP(uint32_t value)
{
    ...
    for (uint8_t i = 0; i < 4; i++)
    {
        ...
        if (i != 3)
            ret += write(FPSTR(JsonConstructor_Dot));
    }
    return ret;
}
...
void JsonConstructor::insert(bool str)
{
    if (_init)
    {
        write(FPSTR(JsonConstructor_Comma));
    }
    else
    {
        _init = true;
    ...
    if (str)
    {
        write(FPSTR(JsonConstructor_Quote));
    }
}
...
void JsonConstructor::push(const char *name, const char *value)
{
    insert();
    write(name);
    write(FPSTR(JsonConstructor_TextValue));
...
void JsonConstructor::push(const char *name, uint32_t value)
{
    insert();
    write(name);
    write(FPSTR(JsonConstructor_Value));
    write(value);
}
