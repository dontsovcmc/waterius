...
bool UserClass::sendNewData(const Settings &settings, const SlaveData &data, const CalculatedData &cdata)
{
    ...
    LOG_INFO(FPSTR(S_SND), "-- START -- " + String( THIS_FUNC_DESCRIPTION));
    ...
    LOG_INFO(FPSTR(S_SND), "JSON size:\t" + String( jsonBody.length()));
    ...
    LOG_INFO(FPSTR(S_SND), "Send HTTP code:\t" + String( responseData.code));
