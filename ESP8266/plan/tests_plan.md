...
- `test_mqtt_command` — что из пришедшего по подписке является командой
  (#421, #422). Устройство подписано на всё своё дерево и публикует туда же,
  поэтому получает назад собственные сообщения: снятие retain у команды
  затирало её же значение пустой строкой, а снятие retain у показаний стирало
  то, из чего Home Assistant берёт значения после перезапуска
