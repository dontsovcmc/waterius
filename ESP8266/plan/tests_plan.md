...
`uint16_t tune_wakeup(time_t now, time_t base_time, time_t last_send, uint16_t wakeup_per_min, uint16_t period_min_tuned)`
...
| PR | Содержание | Меняет поведение |
|----|------------|------------------|
| 1 | `core/types.h`, правка `[env:native]`, починенный `test_ota`, CI, удаление `.travis.yml` | нет |
| 2 | `core/readings.*` + `test_readings` | нет |
| 3 | `core/wakeup.*` + `test_wakeup` | нет |
| 4 | `core/input.*` + `test_input` | нет |
| 5 | `core/routing.*` + `test_routing` | нет |
...
- `test_url` — `get_proto`, `is_https`, `remove_trailing_slash`,
  брокер с протоколом и путём (#330)
- `test_config` — `get_checksum`, `sizeof(Settings) == 960`, версия
  конфигурации, набор полей, переживающих factory reset (#326, #344)
