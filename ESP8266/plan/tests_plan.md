...
> **План исторический.** Разделы «Текущее состояние» и CI ниже описывают август
> 2026. Действующий CI - `.github/workflows/ci.yml`: хостовые тесты ESP в обоих
> окружениях, тесты attiny, строки портала, сборка прошивок и проверка стенда
> без железа (`hil-static`). Тесты на живом железе и их связь с issue -
> `Utils/hil/README.md`.
...
- `test_wakeup` пополнился целым периодом с поправкой (`period_min_full`):
  он не зависит от расстояния до ближайшей точки расписания и потому
  годится для ручного пробуждения кнопкой (#380)
...
- `test_ha` — допустимые пары `device_class` + `unit_of_measurement`. HA молча
  выбрасывает сущность с недопустимой парой: сенсор не появляется, ошибка
  видна только в логе самого HA (#356)
- `test_discovery` — когда переотправлять автообнаружение HA и какие сущности
  нужны входу. Отпечаток обязан меняться от каждого поля, из которого собраны
  конфиги: иначе смена пропадёт до нажатия кнопки. Ненужные сущности входа
  прошивка удаляет, поэтому ошибка в списке стирает из HA живую сущность
- `test_idle` — режим "выходить на связь только при расходе" (#361):
  обнаружение расхода, решение включать ли Wi-Fi, отметка "устройство живо"
  раз в трое суток и интервал, который уезжает на сервер вместо периода
  пробуждения. Период сна этот режим не трогает, поэтому в `test_wakeup`
  ничего не поменялось
- `test_restart` — перезагрузилась ли ЕСП при живом питании (#354): причина
  ресета плюс флаг attiny "питание подано давно". Ложная плашка стоит дороже
  пропущенной, поэтому отдельно закреплено, что байт флагов от старой
  прошивки attiny (< 40) игнорируется целиком, а чужие биты в нём не зовут
  плашку
- `test_diagnostics` — ошибки настройки счётчиков (#283): вес импульса,
  завышенный автоопределением в 10 раз, и вход, который настроили, но он
  ничего не считает. Обе проверки должны молчать, пока не уверены, поэтому
  большая часть тестов закрепляет именно молчание: одинаковые веса,
  отношение ниже порога, слишком малый расход, откат счётчика attiny,
  ненастроенный вход и неводяной ресурс
...
- `test_ha` — расширить: сущности по типам счётчиков, `default_entity_id`
  (#351), публикация `0.0` (#325), стабильность `unique_id`
