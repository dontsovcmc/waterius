...
00:06  OTA: start
00:06  Проверка батареи (3 замера × 100мс)     (~0.2с)
...
ESP8266/
├── src/
│   ├── ota_update.h          # Константы, коды ошибок, объявление perform_ota_update()
...
├── test/
│   └── test_ota/
│       └── test_url_validation.cpp  # Тесты валидации URL (native, googletest)
├── scripts/
...
Тесты покрывают валидацию URL (https:// обязателен, http:// отклоняется, пустой URL, nullptr и др.).
